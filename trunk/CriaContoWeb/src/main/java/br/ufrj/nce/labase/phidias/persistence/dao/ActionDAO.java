...
public class ActionDAO extends GenericDAO<Action> {
	public List<Action> listActions(Integer sessionId, Integer phaseId){
		try {
			Query query = this.getSession().createQuery("select f from Action f where f.sessionGamePhase.id.sessionId = :idSessao and f.sessionGamePhase.id.phaseId = :idFaseJogo order by f.id");
